...
export class EventService {
    ...
    async createEvent(payload: CreateEventPayload): Promise<EventDto> {
        ...
        // categoryId 검증
        const categoryName = await this.eventRepository.getCategoryNameById(payload.categoryId);
        if(!categoryName)
            throw new NotFoundException('입력한 카테고리 ID가 존재하지 않습니다.');

        // cityId 검증
        const cityName = await this.eventRepository.getCityNameById(payload.categoryId);
        if(!cityName)
